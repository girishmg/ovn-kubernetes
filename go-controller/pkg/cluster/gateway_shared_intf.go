...
func cleanupSharedGateway() (error) {
	// NicToBridge() may be created bebore-hand, only delete the patch port here
	stdout, stderr, err := util.RunOVSVsctl("--columns=name", "--no-heading", "find", "port", "external_ids:ovn-localnet-port!=_")
	if err != nil {
