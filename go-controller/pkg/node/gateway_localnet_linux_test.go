...
func initFakeNodePortWatcher(fakeOvnNode *FakeOVNNode, fakeIPT map[iptables.Protocol]util.IPTablesHelper) *localPortWatcherData {
	initIPTable := map[string]util.FakeTable{
		"filter": {},
		"nat":    {},
	}

	f4 := fakeIPT[iptables.ProtocolIPv4].(*util.FakeIPTables)
	f6 := fakeIPT[iptables.ProtocolIPv6].(*util.FakeIPTables)
	Expect(f4.MatchState(initIPTable)).NotTo(HaveOccurred())
	Expect(f6.MatchState(initIPTable)).NotTo(HaveOccurred())
	...
	Context("on startup", func() {
		...
		It("inits physical routing rules", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				fNPW := initFakeNodePortWatcher(fakeOvnNode, fakeIPT)
		...
		It("removes stale physical routing rules while keeping remaining intact", func() {
			app.Action = func(ctx *cli.Context) error {
				...
				fakeRules := externalIPTRules(service.Spec.Ports[0], externalIP, service.Spec.ClusterIP)
				addIptRules(fakeRules)
				fakeRules = externalIPTRules(
					v1.ServicePort{
						Port:     27000,
