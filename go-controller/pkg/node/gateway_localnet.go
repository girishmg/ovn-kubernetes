...
func (npw *localPortWatcherData) networkHasAddress(ipNet net.IP) bool {
...
func (npw *localPortWatcherData) addService(svc *kapi.Service) error {
	...
	for _, port := range svc.Spec.Ports {
		if util.ServiceTypeHasNodePort(svc) {
			...
			iptRules = append(iptRules, nodePortIPTRules(port, npw.gatewayIP, port.NodePort)...)
		...
		for _, externalIP := range svc.Spec.ExternalIPs {
			...
			if _, exists := npw.localAddrSet[externalIP]; exists {
				...
				iptRules = append(iptRules, externalIPTRules(port, externalIP, svc.Spec.ClusterIP)...)
...
func (npw *localPortWatcherData) syncServices(serviceInterface []interface{}) {
	...
	for _, service := range serviceInterface {
		...
		keepIPTRules = append(keepIPTRules, getGatewayIptRules(svc, npw.gatewayIP)...)
