...
func UnmarshalNodeL3GatewayAnnotation(node *kapi.Node) (map[string]string, error) {
	...
	l3GatewayConfig, ok := l3GatewayConfigMap["default"]
	if !ok {
		return nil, fmt.Errorf("%s annotation for %s network not found", OvnNodeL3GatewayConfig, "default")
	}
...
func parseGatewayMacAddress(l3GatewayConfig map[string]string) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("%s annotation not found for", OvnNodeGatewayMacAddress)
	}
...
func (oc *Controller) syncGatewayLogicalNetwork(node *kapi.Node, mode string, l3GatewayConfig map[string]string, subnet string) error {
	var err error
