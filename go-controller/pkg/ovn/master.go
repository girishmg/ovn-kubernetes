...
const (
	...
	OvnNodeManagementPortMacAddress = "k8s.v1.ovn.org/node-mgmt-port-mac-address"
)
...
func parseNodeManagementPortMacAddr(node *kapi.Node) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("Error %v in parsing node macAddress %v", err, macAddress)
	}
...
func (oc *Controller) syncNodeManagementPort(node *kapi.Node) error {
	...
	if macAddress == "" {
		...
		stdout, stderr, err := util.RunOVNNbctl("--", "--may-exist", "lsp-del", node.Name, "k8s-"+node.Name)
		if err != nil {
	...
	if subnet == nil {
		return fmt.Errorf("error obtaining subnet for macAddress %v of node %s", macAddress, node.Name)
	}
	...
	stdout, stderr, err := util.RunOVNNbctl("--", "--may-exist", "lsp-add", node.Name, "k8s-"+node.Name, "--", "lsp-set-addresses", "k8s-"+node.Name, macAddress+" "+portIP)
	if err != nil {
