...
func localGatewayInitTest(fexec *ovntest.FakeExec, nodeName, nodeIP, clusterCIDR, clusterRouter, systemID, localIfaceID, localBrLocalnetMAC, localLocalnetGatewayIP, localLocalnetGatewayNextHop string) {
	const (
		gwLRPMAC string = "00:00:00:05:46:c4"
	...
	fexec.AddFakeCmd(&ovntest.ExpectedCmd{
		Cmd:    "ovn-nbctl --timeout=15 get logical_switch_port jtor-" + localGwRouter + " dynamic_addresses addresses",
		Output: "[]" + "\n" + "[]",
	})
	fexec.AddFakeCmdsNoOutputNoError([]string{
		"ovn-nbctl --timeout=15 --wait=sb --may-exist lsp-add join jtor-" + localGwRouter + " -- --if-exists clear logical_switch_port jtor-" + localGwRouter + " dynamic_addresses -- lsp-set-addresses jtor-" + localGwRouter + " dynamic",
	})
	fexec.AddFakeCmd(&ovntest.ExpectedCmd{
		Cmd:    "ovn-nbctl --timeout=15 get logical_switch_port jtor-" + localGwRouter + " dynamic_addresses addresses",
	...
	fexec.AddFakeCmdsNoOutputNoError([]string{
		"ovn-nbctl --timeout=15 -- --may-exist lrp-add " + localGwRouter + " rtoj-" + localGwRouter + " " + gwLRPMAC + " " + lrpIP + "/16" + " -- set logical_switch_port jtor-" + localGwRouter + " type=router options:router-port=rtoj-" + localGwRouter + " addresses=router",
