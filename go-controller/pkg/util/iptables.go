...
// NewFakeIPTables populates `helpers` with FakeIPTablesHelper that can be used in unit tests
func NewFakeIPTables() map[iptables.Protocol]IPTablesHelper {
	for _, protocol := range []iptables.Protocol{iptables.ProtocolIPv4, iptables.ProtocolIPv6} {
		...
		SetIPTablesHelper(protocol, ipt)
	}
	return helpers
}
