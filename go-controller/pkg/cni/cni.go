...
func (pr *PodRequest) cmdDel(podLister corev1listers.PodLister, kclient kubernetes.Interface) (*Response, error) {
	...
	if pr.CNIConf.DeviceID != "" {
		if config.OvnKubeNode.Mode == types.NodeModeDPUHost {
			...
			if err != nil {
				klog.Errorf("Failed to get pod %s/%s: %v", pr.PodNamespace, pr.PodName, err)
				return response, nil
			}
			dpuCD := util.DPUConnectionDetails{}
			if err := dpuCD.FromPodAnnotation(pod.Annotations); err != nil {
				klog.Errorf("Failed to get DPU connection details annotation for pod %s/%s: %v", pr.PodNamespace,
					pr.PodName, err)
				return response, nil
			}
