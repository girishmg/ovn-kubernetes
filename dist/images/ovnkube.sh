...
cleanup-ovs-server () {
  echo "=============== time: $(date +%d-%m-%H:%M:%S:%N) cleanup-ovs-server (wait for ovn-node to exit) =======" >> /var/log/ovn-kubernetes/ovnkube-cleanup.log
  retries=0
  while [[ ${retries} -lt 80 ]]; do
    if [[ ! -e /var/run/openvswitch/ovnkube.pid ]] ; then
      break
    fi
    echo "=============== time: $(date +%d-%m-%H:%M:%S:%N) cleanup-ovs-server ovn-node still running, wait) =======" >> /var/log/ovn-kubernetes/ovnkube-cleanup.log
    sleep 1
  done
  echo "=============== time: $(date +%d-%m-%H:%M:%S:%N) cleanup-ovs-server (ovs-ctl stop) =======" >> /var/log/ovn-kubernetes/ovnkube-cleanup.log
  /usr/share/openvswitch/scripts/ovs-ctl stop
}
...
cleanup-ovn-node () {
  ...
  echo "=============== time: $(date +%d-%m-%H:%M:%S:%N) cleanup-ovn-node - (wait for ovn-controller to exit)" >> /var/log/ovn-kubernetes/ovnkube-cleanup.log
  retries=0
  while [[ ${retries} -lt 80 ]]; do
    ...
    echo "=============== time: $(date +%d-%m-%H:%M:%S:%N) cleanup-ovn-node - (ovn-controller still running, wait)" >> /var/log/ovn-kubernetes/ovnkube-cleanup.log
    sleep 1
  done

  echo "=============== time: $(date +%d-%m-%H:%M:%S:%N) cleanup-ovn-node   --cleanup-node" >> /var/log/ovn-kubernetes/ovnkube-cleanup.log
  /usr/bin/ovnkube --cleanup-node ${K8S_NODE} --cleanup-gateways ${ovn_gateway_opts} \
      --k8s-token=${k8s_token} --k8s-apiserver=${K8S_APISERVER} --k8s-cacert=${K8S_CACERT} \
      --loglevel=${ovnkube_loglevel} \
      --logfile /var/log/ovn-kubernetes/ovnkube-cleanup.log
